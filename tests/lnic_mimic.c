...
static int process_udp(void *buf, uint8_t *mac) {
	...
	uint64_t word0 = message_data[0].word;
	uint64_t word1 = message_data[1].word;
	uint64_t word2 = message_data[2].word;
	asm volatile("mv t0, %0" : /*no output*/ : /*read*/"r"(word0) : /*clobber*/"t0"); // Word 0 lnic read from t0
	asm volatile("mv t1, %0" : /*no output*/ : /*read*/"r"(word1) : /*clobber*/"t1"); // Word 1 lnic read from t1
	asm volatile("mv t2, %0" : /*no output*/ : /*read*/"r"(word2) : /*clobber*/"t2"); // Word 2 lnic read from t2

	PROFILE_START;
	asm volatile("add t3, t0, 1" : /*no output*/ : /*no input*/ : /*clobber*/"t3"); // Lnic read, add 1, write word 0
	asm volatile("add t4, t1, 1" : /*no output*/ : /*no input*/ : /*clobber*/"t4"); // Lnic read, add 1, write word 1
	asm volatile("add t5, t2, 1" : /*no output*/ : /*no input*/ : /*clobber*/"t5"); // Lnic read, add 1, write word 2
	asm volatile("mv t0, t3" : : : "t0"); // Useless instruction, mimic the message end write
	PROFILE_STOP;

	asm volatile("mv %0, t3" : /*write*/"=r"(word0));
	asm volatile("mv %0, t4" : /*write*/"=r"(word1));
	asm volatile("mv %0, t5" : /*write*/"=r"(word2));
